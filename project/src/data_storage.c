...
void get_Time(char* str){
	sprintf(str,"%02d%c%02d", dateTime.tm_hour, SEC_CHARACTER[dateTime.tm_sec%2], dateTime.tm_min);
}
...
void change_Time(int field, int val){
	(*TIME_PTRS[field])+=val;
	if((*TIME_PTRS[field])<0)(*TIME_PTRS[field])=MAX_TIMES[field]-1;
	(*TIME_PTRS[field])=(*TIME_PTRS[field])%MAX_TIMES[field];
}
